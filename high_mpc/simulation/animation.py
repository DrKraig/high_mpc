...
class SimVisual(object):
    ...
    def init_animate(self,):
        # Initialize position.....
        self.l_quad_px.set_data(np.array([]), [])
        self.l_pend_px.set_data(np.array([]), [])
        self.l_quad_py.set_data(np.array([]), [])
        self.l_pend_py.set_data(np.array([]), [])
        self.l_quad_pz.set_data(np.array([]), [])
        self.l_pend_pz.set_data(np.array([]), [])

        # Initialize velocity.....
        self.l_quad_vx.set_data(np.array([]), [])
        self.l_pend_vx.set_data(np.array([]), [])
        self.l_quad_vy.set_data(np.array([]), [])
        self.l_pend_vy.set_data(np.array([]), [])
        self.l_quad_vz.set_data(np.array([]), [])
        self.l_pend_vz.set_data(np.array([]), [])

        # Initialize attitude.....
        self.l_quad_ax.set_data(np.array([]), [])
        self.l_pend_ax.set_data(np.array([]), [])
        self.l_quad_ay.set_data(np.array([]), [])
        self.l_pend_ay.set_data(np.array([]), [])
        self.l_quad_az.set_data(np.array([]), [])
        self.l_pend_az.set_data(np.array([]), [])

        # Initialize attitude.....
        self.l_quad_thrust.set_data(np.array([]), [])
        self.l_quad_wx.set_data(np.array([]), [])
        self.l_quad_wy.set_data(np.array([]), [])
        self.l_quad_wz.set_data(np.array([]), [])
        
        # Initialize quadrotor 3d trajectory
        self.l_quad_pos.set_data(np.array([]), [])
        self.l_quad_pos.set_3d_properties([])
        # Initialize MPC planned trajectory
        self.l_quad_pred_traj.set_data(np.array([]), [])
        self.l_quad_pred_traj.set_3d_properties([])
        # Initialize planned pendulum trajectory
        self.l_pend_pred_traj.set_data(np.array([]), [])
        self.l_pend_pred_traj.set_3d_properties([])
        # Initialize pendulum plot
        self.l_pend.set_data(np.array([]), [])
        self.l_pend.set_3d_properties([])
        #
        self.l_pend_edge1.set_data(np.array([]), [])
        self.l_pend_edge1.set_3d_properties([])
        self.l_pend_edge2.set_data(np.array([]), [])
        self.l_pend_edge2.set_3d_properties([])
        self.l_pend_edge3.set_data(np.array([]), [])
        self.l_pend_edge3.set_3d_properties([])
        self.l_pend_edge4.set_data(np.array([]), [])
        self.l_pend_edge4.set_3d_properties([])

        # Initialize quad arm
        self.l_quad_x.set_data(np.array([]), [])
        self.l_quad_x.set_3d_properties([])
        self.l_quad_y.set_data(np.array([]), [])
        self.l_quad_y.set_3d_properties([])
        self.l_quad_z.set_data(np.array([]), [])
    ...
    def update(self, data_info):
        ...
        if len(self.ts) == 0:
            ...
            time_s = np.array(self.ts)
            self.l_quad_px.set_data(time_s, quad_pos_arr[:, 0])
            self.l_quad_py.set_data(time_s, quad_pos_arr[:, 1])
            self.l_quad_pz.set_data(time_s, quad_pos_arr[:, 2])
            #
            quad_vel_arr = np.array(self.quad_vel)
            self.l_quad_vx.set_data(time_s, quad_vel_arr[:, 0])
            self.l_quad_vy.set_data(time_s, quad_vel_arr[:, 1])
            self.l_quad_vz.set_data(time_s, quad_vel_arr[:, 2])
            #
            quad_att_arr = np.array(self.quad_att)
            self.l_quad_ax.set_data(time_s, quad_att_arr[:, 0])
            self.l_quad_ay.set_data(time_s, quad_att_arr[:, 1])
            self.l_quad_az.set_data(time_s, quad_att_arr[:, 2])
            # 
            quad_act_arr = np.array(self.quad_cmd)
            self.l_quad_thrust.set_data(time_s, quad_act_arr[:, 0])
            self.l_quad_wx.set_data(time_s, quad_act_arr[:, 1])
            self.l_quad_wy.set_data(time_s, quad_act_arr[:, 2])
            self.l_quad_wz.set_data(time_s, quad_act_arr[:, 3])
                    
            #
            pend_pos_arr = np.array(self.pend_pos)
            self.l_pend_px.set_data(time_s, pend_pos_arr[:, 0])
            self.l_pend_py.set_data(time_s, pend_pos_arr[:, 1])
            self.l_pend_pz.set_data(time_s, pend_pos_arr[:, 2])
            #
            pend_vel_arr = np.array(self.pend_vel)
            self.l_pend_vx.set_data(time_s, pend_vel_arr[:, 0])
            self.l_pend_vy.set_data(time_s, pend_vel_arr[:, 1])
            self.l_pend_vz.set_data(time_s, pend_vel_arr[:, 2])
            #
            pend_att_arr = np.array(self.pend_att)
            self.l_pend_ax.set_data(time_s, pend_att_arr[:, 0])
            self.l_pend_ay.set_data(time_s, pend_att_arr[:, 1])
            self.l_pend_az.set_data(time_s, pend_att_arr[:, 2])
            ...
            #
            self.l_pend.set_data(np.array([self.pivot_point[0], pend_obs[kPosX]]), [self.pivot_point[1], pend_obs[kPosY]] )
            self.l_pend.set_3d_properties([self.pivot_point[2], pend_obs[kPosZ]])
                     
            c1, c2, c3, c4 = pend_corners
            #
            self.l_pend_edge1.set_data(np.array([c1[0], c2[0]]), [c1[1], c2[1]])
            self.l_pend_edge1.set_3d_properties([c1[2], c2[2]])
            self.l_pend_edge2.set_data(np.array([c2[0], c2[0]]), [c2[1], c4[1]])
            self.l_pend_edge2.set_3d_properties([c2[2], c4[2]])
            self.l_pend_edge3.set_data(np.array([c4[0], c3[0]]), [c4[1], c3[1]])
            self.l_pend_edge3.set_3d_properties([c4[2], c3[2]])
            self.l_pend_edge4.set_data(np.array([c3[0], c1[0]]), [c3[1], c1[1]])
            ...
            for _, quad_motor in enumerate(self.quad_hist):
                ...
                self.l_quad_x.set_data(np.array([quad_pos_arr[-1, 0], axes_x[0]]), [quad_pos_arr[-1, 1], axes_x[1]])
                self.l_quad_x.set_3d_properties([quad_pos_arr[-1, 2], axes_x[2]])
                self.l_quad_y.set_data(np.array([quad_pos_arr[-1, 0], axes_y[0]]), [quad_pos_arr[-1, 1], axes_y[1]])
                self.l_quad_y.set_3d_properties([quad_pos_arr[-1, 2], axes_y[2]])
                self.l_quad_z.set_data(np.array([quad_pos_arr[-1, 0], axes_z[0]]), [quad_pos_arr[-1, 1], axes_z[1]])
...
class SimVisual2(object):
    ...
    def __init__(self, env):
        ...
        # and figure grid
        self.gs = gridspec.GridSpec(nrows=2, ncols=1)
        
        # Create layout of our plots
        self.ax_pos = self.fig.add_subplot(self.gs[0, 0])
        self.ax_pos.set_ylim([self.pos_min, self.pos_max])
        self.ax_pos.set_xlim([self.t_min, self.t_max])
        self.ax_pos.set_title("Quad and Pendulum Trajectories")
        
        self.ax_act = self.fig.add_subplot(self.gs[1, 0])
        self.ax_act.set_ylim([self.act_min, self.act_max])
        self.ax_act.set_xlim([0, self.t_max])
        
        self.ax_act.set_title("Quad Actions")
        ...
        self.ax_pos.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        self.ax_act.legend(loc='center left', bbox_to_anchor=(1, 0.5))
